...
void __ISR(_TIMER_4_VECTOR, IPL4SOFT) Timer4ISR(void) {
  // code for PI control goes here
    ...
    //error from phone: run a P loop to determine the reference velocity to feed into the PI controller
    error = rxVal - 240; // 240 means the dot is in the middle of the screen
        if (error<0) { // slow down the left motor to steer to the left
        error  = -error;
        ref_vel5 = MAX_DUTY - Kp_camera * error;
        ref_vel3 = MAX_DUTY;
            if (ref_vel5 < 0){
            ref_vel5 = 0;
             }
        ...
        ref_vel3 = MAX_DUTY - Kp_camera *error;
        ref_vel5 = MAX_DUTY;
            if (ref_vel3<0) {
                ref_vel3 = 0;
                }
